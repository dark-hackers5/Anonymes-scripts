--[[
    ANONYMES SCRIPTS HUB - Blox Fruits (Version ULTIME 4.1)
    Optimisé pour Delta Executor | Compatible : Synapse X, Krnl, Fluxus, Script-Ware
    ✅ Logo immédiat | ✅ Anti-Crash | ✅ 100% des fonctionnalités du jeu
--]]
...
local LOGO_URL = "https://i.imgur.com/8Q5y7zD.png" -- Remplace par ton URL si besoin
...
local toggles = {
    ...
    autoGifts = false, autoCollect = false, autoCollectRewards = false,
    -- Combat
    autoPvP = false, killAura = false, triggerBot = false, autoAttack = false,
    -- Events
    autoSeaBeast = false, autoLeviathan = false, autoLeviathanQuest = false,
    autoPirateRaid = false, autoShipRaid = false, autoTerrorshark = false,
    autoVolcano = false, autoFactory = false, autoPhantom = false,
    -- Quêtes
    autoDoughKing = false, autoYama = false, autoTushita = false, autoCDK = false,
    autoSanguineArt = false, autoKitsune = false, autoRaceV4 = false,
    autoMirageIsland = false, autoSoulReaper = false,
    -- Fruits & Raid
    fruitsPerms = false, autoRaid = false, espFruits = false,
    -- Boats
    autoBoat = false, teleportToBoat = false, boatSpeed = false,
    -- Teleport
    teleportMobs = false, bringMob = false, autoIslandHop = false,
    -- ESP
    espPlayers = false, espBoss = false, espChests = false, espNPCs = false,
    -- Movement
...
local function payBeli(amount)
    ...
    pcall(function() player.Data.Beli.Value = player.Data.Beli.Value - amount end)
    return true
end
...
local function startAutoLevel()
    ...
    spawn(function()
        while toggles.autoLevel and task.wait(0.3) do
            pcall(function() local mob = getNearestMob(); if mob then attackTarget(mob) end end)
        end
    end)
end
    ...
    spawn(function()
        while toggles.autoNearest and task.wait(0.5) do
            pcall(function()
                ...
                else local mob = getNearestMob(); if mob then attackTarget(mob) end end
            end)
        end
    end)
end
...
local function startAutoSeaBeast()
    ...
    spawn(function()
        while toggles.autoSeaBeast and task.wait(1) do
            pcall(function() local seaBeast = getBoss("Sea Beast"); if seaBeast then attackTarget(seaBeast) end end)
        end
    end)
end
...
local function startAutoLeviathan()
    ...
    spawn(function()
        while toggles.autoLeviathan and task.wait(1) do
            pcall(function() local leviathan = getBoss("Leviathan"); if leviathan then attackTarget(leviathan) end end)
        end
    end)
end
            ...
            pcall(function()
                ...
                if oldMan and oldMan:FindFirstChild("HumanoidRootPart") then
                    ...
                    for _, waveName in ipairs({"LeviathanWave1", "LeviathanWave2", "LeviathanWave3"}) do
                        ...
                        if wave then attackTarget(wave); task.wait(0.5) end
                    end
...
local function startAutoTerrorshark()
    ...
    spawn(function()
        while toggles.autoTerrorshark and task.wait(1) do
            pcall(function() local terrorshark = getBoss("Terrorshark"); if terrorshark then attackTarget(terrorshark) end end)
        end
    end)
end
...
local function startAutoVolcano()
    ...
    spawn(function()
        while toggles.autoVolcano and task.wait(1) do
            pcall(function() local magmaAdmiral = getBoss("Magma Admiral"); if magmaAdmiral then attackTarget(magmaAdmiral) end end)
        end
    end)
end
...
local function startAutoFactory()
    ...
    spawn(function()
        while toggles.autoFactory and task.wait(1) do
            pcall(function() local mirrorBoss = getBoss("Mirror Boss"); if mirrorBoss then attackTarget(mirrorBoss) end end)
        end
    end)
end
...
local function startAutoPhantom()
    ...
    spawn(function()
        while toggles.autoPhantom and task.wait(1) do
            pcall(function() local phantom = getBoss("Phantom"); if phantom then attackTarget(phantom) end end)
        end
    end)
end
    ...
    spawn(function()
        while toggles.autoSanguineArt and task.wait(1) do
            pcall(function()
                ...
                if sanguine then attackTarget(sanguine)
                else
    ...
    spawn(function()
        while toggles.autoKitsune and task.wait(1) do
            pcall(function()
                ...
                if kitsune then attackTarget(kitsune)
                else
    ...
    spawn(function()
        while toggles.autoSoulReaper and task.wait(1) do
            pcall(function()
                ...
                if soulReaper then attackTarget(soulReaper)
                else
...
-- ========== [FONCTIONS D'ÎLE MIrage + COLLECTE] ==========
                    ...
                    if mirageNPC and mirageNPC:FindFirstChild("HumanoidRootPart") then
                        ...
                        for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                            if enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 and enemy:FindFirstChild("HumanoidRootPart") then
                                ...
                                if dist < 1000 then attackTarget(enemy); task.wait(0.5) end
                            end
                        end
                        if mirageNPC then
                            rootPart.CFrame = mirageNPC.HumanoidRootPart.CFrame * CFrame.new(0, 0, 5)
                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Talk", mirageNPC)
                            notify("✅ Quête Île Mirage", "Terminée !", 5)
                        end
                    end
                end
            end)
...
local function startAutoCollectRewards()
    ...
    spawn(function()
        while toggles.autoCollectRewards and task.wait(30) do
            pcall(function()
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CollectAll")
                notify("✅ Récompenses", "Collectées !", 2)
            end)
        end
    end)
end

-- ========== [FONCTIONS DE FRUITS PERMS] ==========
...
local function buyFeature(feature, cost)
    if isVIP then
        ...
        if (feature == "fruitsPerms" and hasFruitsPerms) or (feature == "autoPvP" and hasAutoPvP) then
            notify("ℹ️ Déjà débloqué", "Tu as déjà cette fonctionnalité.", 3)
            return true
        end
...
local function createUI()
    ...
    local tabButtons = {}
    local tabFrames = {}
    local tabNames = {"Home", "Farming", "Combat", "Events", "Quêtes", "Boats", "Îles", "ESP", "Haki", "Movement", "Admin"}
    ...
    local farmingFeatures = {
        {Name = "autoLevel", Display = "Auto Level"},
        {Name = "autoNearest", Display = "Auto Nearest (Boss/Mob)"},
        {Name = "autoEctoplasm", Display = "Auto Ectoplasm"},
        {Name = "autoCandy", Display = "Auto Candy"},
        {Name = "autoGifts", Display = "Auto Gifts"},
        {Name = "autoCollect", Display = "Auto Collect Fruits"},
        {Name = "autoCollectRewards", Display = "Auto Collect Rewards"}
    }
    for _, feature in ipairs(farmingFeatures) do
        ...
        button.MouseButton1Click:Connect(function()
            ...
            if newState then
                ...
                elseif feature.Name == "autoCollectRewards" then startAutoCollectRewards() end
            end
        end)
    end
    ...
    local combatFeatures = {
        {Name = "killAura", Display = "Kill Aura"},
        {Name = "triggerBot", Display = "TriggerBot"},
        {Name = "autoAttack", Display = "Auto Attack"},
        {Name = "autoPvP", Display = "Auto PvP"}
    }
        ...
        button.MouseButton1Click:Connect(function()
            if feature.Name == "autoPvP" then
                ...
                if newState then
                    ...
                    elseif feature.Name == "autoAttack" then startAutoAttack() end
                end
            end
        end)
    end
    ...
    local eventFeatures = {
        {Name = "autoSeaBeast", Display = "Auto Sea Beast"},
        {Name = "autoLeviathan", Display = "Auto Leviathan (Boss seul)"},
        {Name = "autoLeviathanQuest", Display = "⭐ Auto Leviathan (Quête complète)"},
        {Name = "autoPirateRaid", Display = "Auto Pirate Raid"},
        {Name = "autoShipRaid", Display = "Auto Ship Raid"},
        {Name = "autoTerrorshark", Display = "Auto Terrorshark"},
        {Name = "autoVolcano", Display = "Auto Volcano"},
        {Name = "autoFactory", Display = "Auto Factory"},
        {Name = "autoPhantom", Display = "Auto Phantom"}
    }
    for _, feature in ipairs(eventFeatures) do
        ...
        button.MouseButton1Click:Connect(function()
            ...
            if newState then
                ...
                elseif feature.Name == "autoPhantom" then startAutoPhantom() end
            end
        end)
    end
    ...
    local questFeatures = {
        {Name = "autoDoughKing", Display = "Auto Dough King"},
        {Name = "autoYama", Display = "Auto Yama"},
        {Name = "autoTushita", Display = "Auto Tushita"},
        {Name = "autoCDK", Display = "Auto Cursed Dual Katana"},
        {Name = "autoSanguineArt", Display = "Auto Sanguine Art"},
        {Name = "autoKitsune", Display = "Auto Kitsune"},
        {Name = "autoRaceV4", Display = "Auto Race V4"},
        {Name = "autoMirageIsland", Display = "⭐ Auto Mirage Island"},
        {Name = "autoSoulReaper", Display = "Auto Soul Reaper"}
    }
    for _, feature in ipairs(questFeatures) do
        ...
        button.MouseButton1Click:Connect(function()
            ...
            if newState then
                ...
                elseif feature.Name == "autoSoulReaper" then startAutoSoulReaper() end
            end
        end)
    end
    ...
    local boatFeatures = {
        {Name = "autoBoat", Display = "Auto Conduire Bateau"},
        {Name = "teleportToBoat", Display = "Téléport vers Bateau"},
        {Name = "boatSpeed", Display = "Boat Speed Hack (200)"}
    }
        ...
        button.MouseButton1Click:Connect(function()
            if feature.Name == "teleportToBoat" then
                ...
                if newState then
                    ...
                    elseif feature.Name == "boatSpeed" then boostBoatSpeed() end
                end
            end
        end)
    end
    ...
    local espFeatures = {
        ...
        {Name = "espBoss", Display = "ESP Boss (niveaux + distance)", Color = Color3.new(1, 0, 0)},
    ...
    local hakiFeatures = {
        {Name = "autoHaki", Display = "Auto Haki (Complet)"},
        {Name = "autoObservation", Display = "Auto Observation Haki (Ken)"},
        {Name = "autoArmament", Display = "Auto Armament Haki (Busoshoku)"}
    }
    for _, feature in ipairs(hakiFeatures) do
        ...
        button.MouseButton1Click:Connect(function()
            ...
            if newState then
                ...
                elseif feature.Name == "autoArmament" then startAutoArmament() end
            end
        end)
    end
    ...
    local movementFeatures = {
        {Name = "walkOnWater", Display = "Walk On Water"},
        {Name = "infiniteJump", Display = "Infinite Jump"},
        {Name = "noClip", Display = "No Clip"},
        {Name = "flight", Display = "Flight (WASD + Espace/Shift)"}
    }
    for _, feature in ipairs(movementFeatures) do
        ...
        button.MouseButton1Click:Connect(function()
            ...
            if newState then
                ...
                elseif feature.Name == "flight" then enableFlight() end
            end
        end)
    end
    ...
    local adminFeatures = {
        {Name = "serverHop", Display = "Server Hop (60s)"},
        {Name = "antiKick", Display = "Anti Kick / Anti Ban"},
        {Name = "fpsBoost", Display = "FPS Boost"}
    }
    for _, feature in ipairs(adminFeatures) do
        ...
        button.MouseButton1Click:Connect(function()
            ...
            if newState then
                ...
                elseif feature.Name == "fpsBoost" then startFPSBoost() end
            end
        end)
    end
...
pcall(function()
    ...
    createUI()
    task.wait(1)
    notify("✅ ANONYMES SCRIPTS HUB v" .. VERSION, "Logo en haut à gauche → **CLIQUE DESSUS** pour ouvrir le menu et farmer.", 10)
end)
