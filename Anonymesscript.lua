...
local VERSION = "13.0"
local LOGO_URL = "rbxassetid://19964848466" -- ✅ TON LOGO "ANONYME SCRIPTS" hébergé

-- =============================================
-- IMPLEMENTATION LOCALE DE DELTA (plus de dépendance externe)
-- =============================================
local Delta = {}
...
-- =============================================
-- CONFIGURATION (TAILLES, COULEURS, TOGGLES)
-- =============================================
local SIZES = {
...
local function hasItem(itemName)
    ...
    return success and has or false
end
...
local function acceptQuest(questName)
    ...
    if success then
        notify("✅ Quête", "Quête '" .. questName .. "' acceptée !", 2)
        return true
    end
    return false
end
    ...
    pcall(function()
        ...
        local success, result = pcall(function()
            if itemType == "Sword" then
                return CommF:InvokeServer("BuySword", itemName)
            elseif itemType == "Gun" then
                return CommF:InvokeServer("BuyGun", itemName)
            end
        end)
        ...
        if success then
            ...
            notify("❌ Erreur", "Impossible d'acheter " .. itemName, 2)
        end
    end)
end
            ...
            pcall(function()
                ...
                if target and target.Character then
                    ...
                    if toggles.autoBounty.Value then
                        CommF:InvokeServer("Bounty", target)
                    end
                end
            end)
            task.wait(0.3)
...
local function storeFruit()
    pcall(function()
        ...
        if fruit ~= "None" then
            ...
            notify("⚠️ Fruit", "Aucun fruit équipé", 2)
        end
    end)
end
                    ...
                    for _, plr in pairs(Players:GetPlayers()) do
                        if plr ~= player and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                            ...
                            if not esp then
                                ...
                                text.Parent = esp -- ✅ CORRIGÉ: esp au lieu de es
                            end
                        end
                    end
                end
                ...
                if toggles.espBoss.Value and workspace:FindFirstChild("Enemies") then
                    for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                        if enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
                            local isBoss = false
                            for _, bName in ipairs(bossListESP) do
...
-- =============================================
-- ACHAT DES ÉPÉES/GUNS (BOUTIQUE)
-- =============================================
local swordList = {
    "Cutlass", "Katana", "Dual Katana", "Longsword", "Iron Mace", "Pipe",
    "Triple Katana", "Dual-Headed Blade", "Bisento", "Soul Cane", "Warden's Sword",
    "Wando", "Saddi", "Shisui", "Midnight Blade", "Dark Dagger", "Tushita",
    "Twin Hooks", "Yama", "Buddy Sword", "Canvander", "Rengoku", "Pole (2nd Form)",
    "Fox Lamp", "Shark Anchor", "Crystal Katana"
}

local gunList = {
    "Flintlock", "Musket", "Blunderbuss", "Cannon", "Refined Slingshot",
    "Dual Flintlock", "Magma Blaster", "Acidum Rifle", "Bazooka",
    "Bizarre Revolver", "Kabucha", "Venom Bow", "Dragonstorm"
}
...
local function buyAllSwords()
    spawn(function()
        while toggles.buyAllSwords.Value do
            ...
            notify("✅ Épées", "Toutes les épées achetées !", 3)
...
local function buyAllGuns()
    spawn(function()
        while toggles.buyAllGuns.Value do
            ...
            notify("✅ Guns", "Tous les guns achetés !", 3)
...
local individualBuys = {
    buyCutlass = {"Cutlass", "Sword"},
    buyKatana = {"Katana", "Sword"},
    buyDualKatana = {"Dual Katana", "Sword"},
    buyLongsword = {"Longsword", "Sword"},
    buyFlintlock = {"Flintlock", "Gun"},
    buyMusket = {"Musket", "Gun"},
    buyBlunderbuss = {"Blunderbuss", "Gun"},
    buyCannon = {"Cannon", "Gun"},
    buyRefinedSlingshot = {"Refined Slingshot", "Gun"},
    buyDualFlintlock = {"Dual Flintlock", "Gun"},
    buyBisento = {"Bisento", "Sword"}
}

for key, item in pairs(individualBuys) do
    if toggles[key] then
        toggles[key]:Connect(function(v)
            if v then
                buyItem(item[1], item[2])
                toggles[key]:Set(false)
            end
        end)
    end
end
...
-- FONCTIONS FRUITS
-- =============================================
toggles.autoStoreFruit:Connect(function(v)
    if v then
        storeFruit()
        toggles.autoStoreFruit:Set(false)
    end
end)

toggles.autoRandomFruit:Connect(function(v)
    if v then
        buyRandomFruit()
        toggles.autoRandomFruit:Set(false)
    end
end)
