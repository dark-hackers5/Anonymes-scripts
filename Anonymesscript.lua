            ...
            if T.autoCollectFruit then
                for _, obj in pairs(workspace:GetChildren()) do
                    if (obj:IsA("Tool") or obj.Name:find("Fruit")) and obj:FindFirstChild("Handle") then
                        ...
                        firetouchinterest(getR(), obj.Handle, 0); task.wait(0.05); firetouchinterest(getR(), obj.Handle, 1)
                    end
                end
            end
...
ttitle.Text = "◆ ANONYMES SCRIPT ◆ v"..VERSION; ttitle.TextSize = 18
...
cscroll.ScrollBarImageColor3 = C.primary; cscroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
...
local function openCategory(cat)
    ...
    if cat.name == "BOSSES" then
        ...
        Instance.new("UIStroke", allbtn).Color = C.warning
        Instance.new("UIPadding", allbtn).PaddingLeft = UDim.new(0,10)
        ...
        Instance.new("UIPadding", tit).PaddingLeft = UDim.new(0,10)local bs = Instance.new("ScrollingFrame", cont)
        bs.Size = UDim2.new(1,-10,0,280); bs.BackgroundColor3 = C.bg2; bs.BackgroundTransparency = 0.3
        bs.ScrollBarThickness = 3; bs.AutomaticCanvasSize = Enum.AutomaticSize.Y
    ...
    elseif cat.name == "TELEPORT" then
        ...
        ts.ScrollBarThickness = 3; ts.AutomaticCanvasSize = Enum.AutomaticSize.Y
...
for _, cat in ipairs(CATEGORIES) do
    ...
    btn.Selected = false
    btn.MouseButton1Click:Connect(function()
        for _, b in pairs(catBtns) do b.Selected = false; b.BackgroundColor3 = C.card; b.TextColor3 = C.textMuted; s.Transparency = 0.7 end
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "🚀 Anonymes Script v"..VERSION,
